...
const findGenre = (
 ...
 var found = genreRows.find(gR => gR.genre == genre);
 return found === undefined ? 0 : found.id;
};
*/
...
const findInArray = (
 ...
 var found = array.find(element => element[property] == thingToFind);
 return found === undefined ? 0 : found.id;
};
...
genres.map(genre => `('${movieId}', '${findInArray('genre', genre, genreRows)}')`).join(",");
};
...
const insertMovieActors = (
  ...
  actors.map(actor => `('${movieId}', '${findInArray('full_name', actor, actorRows)}')`).join(",");
};
...
const insertMovieDirectors = (
  ...
  directors.map(director => `('${movieId}', '${findInArray('full_name', director, directorRows)}')`).join(",");
};
...
const insertMovieKeywords = (
  ...
  keywords.map(keyword => `('${movieId}', '${findInArray('keyword', keyword, keywordRows)}')`).join(",");
};
...
const insertMovieProductionCompanies = (
  ...
  productionCompanies.map(company => `('${movieId}', '${findInArray('company_name', company, productionCompanyRows)}')`).join(",");
};
